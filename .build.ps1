...
#Requires -Modules @{ ModuleName="InvokeBuild"; ModuleVersion="5.4.2" }
...
Enter-Build {
    ...
    if (-not (Get-Variable -Name nuget -Scope Script -ErrorAction Ignore))
    {
        if (-not (($Script:nuget = Get-Command -Name (Join-Path -Path $BuildRoot -ChildPath '.tools/nuget.exe') -ErrorAction Ignore)))
        {
            #exec { & (Join-Path -Path $BuildRoot -ChildPath 'init.cmd') }

            #$Script:nuget = Get-Command -Name (Join-Path -Path $BuildRoot -ChildPath '.tools/nuget.exe')
        }

        $Script:NugetConfigPath = Join-Path -Path $BuildRoot -ChildPath 'nuget.config'
    }
...
$script:moduleName = 'mockONTAP'
task InstallDependencies {
    if (-not (Get-Module -ListAvailable pester))
    {
        Install-Module -Name pester -MinimumVersion 4.4.4 -Repository PSGallery -Force -AllowClobber

    }

    Import-Module -Name pester -MinimumVersion 4.4.4
}
...
task Stage InstallDependencies, Clean, ModuleVersion, {
    ...
    Write-Build Yellow 'Copying files to stage location'

    if (-not (Test-Path -Path "$moduleOutputPath\mockONTAP.psm1" ))
    {
        [void]( New-Item -ItemType File -Path "$moduleOutputPath\mockONTAP.psm1" -Force )
    }
    #Invoke-CopyFileListFromDevLocation -ModuleName $script:moduleName -ModuleOutputPath $moduleOutputPath -BuildVariables $Script:BuildVariables
    Get-ChildItem -Path "$BuildRoot\src" -Filter '*.ps1' -Recurse | Get-Content -Raw -Force | Add-Content -Path "$moduleOutputPath\mockONTAP.psm1" -Force

    Copy-Item -Path "$BuildRoot\src\mockONTAP.psd1" -Destination $moduleOutputPath

    Write-Build Yellow 'Updating module version strings in relevant files'
    ...
    Write-Build Yellow 'Testing integrity of module manifest'
...
task Package ModuleVersion, {
    ...
    if (-not (Get-PSRepository -Name local -ErrorAction 'Continue'))
    {
